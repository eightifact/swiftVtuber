...
class ViewController: UIViewController, ARSessionDelegate {
    ...
    override func viewDidLoad() {
        ...
        // add a plane to block the camera view of background
        

        
         let backgroundImage = UIImage(named: "backgroundImageLight")
             let backgroundNode = SCNNode(geometry: SCNPlane(width: 10.0, height: 10.0))
             backgroundNode.geometry?.firstMaterial?.diffuse.contents = backgroundImage
             backgroundNode.geometry?.firstMaterial?.isDoubleSided = true
         backgroundNode.position.z = -8
         sceneView.pointOfView?.addChildNode(backgroundNode)
